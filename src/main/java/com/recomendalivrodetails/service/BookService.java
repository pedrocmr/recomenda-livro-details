...
public class BookService {
    ...
    public List<BookInfo> getBookInfo(List<String> bookTitles) {
        ...
        for (String title : bookTitles) {
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                bookInfoList.addAll(mapToBookInfo(response.getBody()));
            }
        }
        return bookInfoList;
    }
    ...
    private List<BookInfo> mapToBookInfo(GoogleBooksResponse response) {
        ...
        response.getItems().forEach(item -> {
            bookInfoList.add(BookInfo.builder()
                    ...
                    .thumbnailUrl(item.getVolumeInfo().getImageLinks().getThumbnailUrl())
                    .build());
        });
        return bookInfoList;
    }
